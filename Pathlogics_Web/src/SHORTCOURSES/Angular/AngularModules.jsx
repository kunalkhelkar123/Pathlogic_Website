import React, { useState } from 'react';

const AccordionItem = ({ title, content, isOpen, onToggle }) => {
  return (
    <div className="mb-4 border border-gray-200 rounded-lg overflow-hidden shadow-md">
      <button
        className="w-full bg-gradient-to-r from-gray-900 to-teal-600 text-white px-6 py-4 flex justify-between items-center hover:from-gray-800 hover:to-teal-500 transition-all duration-300"
        onClick={onToggle}
      >
        <h3 className="text-xl font-semibold">{title}</h3>
        <svg
          className={`w-6 h-6 transform transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      <div
        className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}
      >
        <div className="bg-white px-6 py-4 text-gray-700 leading-relaxed">{content}</div>
      </div>
    </div>
  );
};

export default function AngularModules() {
  const [openSection, setOpenSection] = useState(null);

  const modules = [
    {
      title: "Introduction to Angular",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>What is Angular?</li>
          <li>Angular Architecture</li>
          <li>Components & Templates</li>
          <li>Data Binding & Directives</li>
        </ul>
      )
    },
    {
      title: "Angular Directives & Pipes",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Understanding Structural & Attribute Directives</li>
          <li>Built-in Pipes</li>
          <li>Creating Custom Directives & Pipes</li>
          <li>Handling User Input</li>
        </ul>
      )
    },
    {
      title: "Angular Router",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Setting Up Angular Router</li>
          <li>Lazy Loading Modules</li>
          <li>Route Guards & Authentication</li>
          <li>Nested & Dynamic Routes</li>
        </ul>
      )
    },
    {
      title: "State Management with NgRx",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Understanding NgRx Store</li>
          <li>Actions, Reducers & Effects</li>
          <li>Selectors & Optimizing State</li>
          <li>Handling Asynchronous State</li>
        </ul>
      )
    },
    {
      title: "Performance Optimization",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Lazy Loading & Code Splitting</li>
          <li>Change Detection Strategies</li>
          <li>Optimizing Angular Applications</li>
          <li>Angular Universal (SSR)</li>
        </ul>
      )
    },
    {
      title: "Testing & Debugging",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Unit Testing with Jasmine & Karma</li>
          <li>End-to-End Testing with Protractor</li>
          <li>Debugging in Angular DevTools</li>
          <li>Performance Profiling</li>
        </ul>
      )
    },
    {
      title: "Final Angular Project",
      content: (
        <ul className="list-disc pl-5 space-y-2">
          <li>Building a Full-Stack Angular App</li>
          <li>Integrating RESTful APIs</li>
          <li>Deploying with Firebase & Vercel</li>
          <li>Implementing Authentication with JWT</li>
        </ul>
      )
    }
  ];

  return (
    <div className="min-h-screen bg-gray-100 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-5xl font-extrabold text-center text-orange-600 mb-12">Advanced Angular Developer Program</h1>
        <div className="space-y-6">
          {modules.map((module, index) => (
            <AccordionItem
              key={index}
              title={module.title}
              content={module.content}
              isOpen={openSection === index}
              onToggle={() => setOpenSection(openSection === index ? null : index)}
            />
          ))}
        </div>
      </div>
    </div>
  );
}
