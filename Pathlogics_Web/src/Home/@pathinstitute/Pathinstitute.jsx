  ...
  return (
    <header className="w-full">
      <div className="bg-black text-white py-2 px-4 md:px-8 flex flex-col md:flex-row justify-between items-center">
        ...
        <div className="hidden sm:flex space-x-4 sm:space-x-6 mt-2 sm:mt-0 ml-auto">
          <div className="relative">
            <button
              onClick={() => setIsDropdownOpen1((prevState) => !prevState)}
              className="font-medium dropdown-button"
            >
              Login
            </button>
            {isDropdownOpen1 && (
              <div className="absolute left-0 mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg dropdown-menu" style={{ zIndex: 222 }}>
                <ul className="space-y-2 p-2">
                  {LoginBar.map((item, index) => (
                    <li key={index}>
                      <Link to={item.path} className="block text-gray-700 hover:text-orange-500 px-4 py-2 text-sm">
                        {item.name}
                      </Link>
                    </li>
                  ))}
      ...
      <div className="w-full bg-white py-1 px-4 md:px-8 flex justify-between items-center border-b-2 border-orange-500">
        <div className="flex items-center">
          <button onClick={() => Navigate('/')} className="text-black focus:outline-none">
            <img src={image} alt="Pathology Logo" />
          </button>
        </div>
        <div className="md:hidden">
          <button onClick={toggleMenu} className="text-black focus:outline-none">
        ...
        <nav className="hidden md:flex space-x-2 sm:space-x-4 md:space-x-6 ml-auto">
          ...
          <div className="relative">
            <button
              onClick={() => setIsDropdownOpen((prevState) => !prevState)}
              className="text-gray-700 hover:text-orange-500 font-medium dropdown-button"
            >
              Short Term Courses
            </button>
            {isDropdownOpen && (
